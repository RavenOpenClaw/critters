...
class Stick(WorldObject):
    def __init__(self, gx, gy, cell_size, sticks=2):
        inventory = Inventory()
        inventory.add(ITEM_WOOD, sticks)
    ...
    def interact(self, player):
        ...
        if self.inventory.has(ITEM_WOOD, 1):
            mult = player.get_gather_multiplier()
            qty = max(1, int(round(mult)))
            available = self.inventory.get_item_count(ITEM_WOOD)
            taken = min(qty, available)
            self.inventory.remove(ITEM_WOOD, taken)
            player.inventory.add(ITEM_WOOD, taken)
