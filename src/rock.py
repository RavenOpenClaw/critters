...
from constants import ITEM_STONE, PROMPT_MINE

class Rock(WorldObject):
    def __init__(self, gx, gy, cell_size, stone=3):
        inventory = Inventory()
        inventory.add(ITEM_STONE, stone)
    ...
    def interact(self, player):
        ...
        if self.inventory.has(ITEM_STONE, 1):
            mult = player.get_gather_multiplier()
            qty = max(1, int(round(mult)))
            available = self.inventory.get_item_count(ITEM_STONE)
            taken = min(qty, available)
            self.inventory.remove(ITEM_STONE, taken)
            player.inventory.add(ITEM_STONE, taken)
