...
Fix Details:
- Introduced `PLANT_RADIUS = 10` in `src/constants.py`.
- Implemented expanding ring search in `Critter._update_plant` to favor closer valid spots.
...
Fix Details:
- Added re-validation check upon arrival at `goal_cell` in `_update_plant`.
- Added final re-validation check immediately before object creation and world addition.
- Critter now clears `goal_cell` and searches again if the spot is invalidated.
...
Fix Details:
- Verified `world.add_object` correctly handles grid registration via `grid.register`.
- Ensured `Critter._update_plant` checks the return value of `world.add_object` and correctly handles the removal of the sapling from inventory only upon successful placement.
