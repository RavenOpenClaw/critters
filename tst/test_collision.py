...
class TestCollisionDetection(unittest.TestCase):
    ...
    def test_sliding_preserves_other_axis_when_one_blocked(self):
        ...
        grid.register(obj1)
        grid.register(obj2)
        player = Player(1.3, 0.3, radius=0.6, speed=1.0)  # start with y=0.3 to avoid initial collision with obj2
        player.move(1, 1, 1.0, grid=grid)
        self.assertEqual(player.x, 1.3)  # X blocked by wall at x=2
        self.assertEqual(player.y, 0.3)  # Y blocked by obstacle below
