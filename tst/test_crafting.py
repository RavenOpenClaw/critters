...
class TestCraftingSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.recipe = Recipe("Test Axe", "test_axe", {"berry": 5}, unlocks_equipment=True)
    ...
    def test_craft_fails_with_insufficient_resources(self):
        ...
        self.player.inventory.add('berry', 3)
        result = self.menu.craft_selected(self.player, self.recipe)
        self.assertFalse(result)
        self.assertNotIn("test_axe", self.player.unlocked_equipment)
        self.assertEqual(self.player.inventory.get_item_count('berry'), 3)
        self.assertIn("Not enough berry!", self.menu.last_message)
    ...
    def test_craft_multiple_resource_types(self):
        ...
        recipe = Recipe("Multi Tool", "multi_tool", {"berry": 3, "wood": 2}, unlocks_equipment=True)
        player = Player(0, 0)
        player.inventory.add('berry', 5)
        ...
        self.assertEqual(player.inventory.get_item_count('berry'), 2)
    ...
    def test_crafting_success_depends_on_sufficiency(self, available, required):
        ...
        recipe = Recipe("PropEquip", "prop_equip", {"berry": required}, unlocks_equipment=True)
        player = Player(0, 0)
        if available >= required:
            player.inventory.add('berry', available)
            menu = CraftingMenu([recipe])
            result = menu.craft_selected(player, recipe)
            self.assertTrue(result)
            self.assertIn("prop_equip", player.unlocked_equipment)
            self.assertEqual(player.inventory.get_item_count('berry'), available - required)
        else:
            player.inventory.add('berry', available)
            menu = CraftingMenu([recipe])
            result = menu.craft_selected(player, recipe)
            ...
            self.assertEqual(player.inventory.get_item_count('berry'), available)
